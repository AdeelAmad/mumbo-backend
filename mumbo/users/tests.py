...
class CountTestCase(TestCase):
    ...
    def test_get_birthday_no_auth(self):
        response = self.client.get('/users/birthday/')
        self.assertEqual(response.status_code, 302)

    def test_get_birthday_incorrect_auth(self):
        credentials = f"{self.username}:eeeeeeeeee"
        base64_credentials = base64.b64encode(credentials.encode('utf8')).decode('utf8')
        response = self.client.patch('/users/birthday/', HTTP_AUTHORIZATION=f"Basic {base64_credentials}")
        self.assertEqual(response.status_code, 302)

    def test_bad_method_birthday_correct_auth(self):
        credentials = f"{self.username}:{self.password}"
        base64_credentials = base64.b64encode(credentials.encode('utf8')).decode('utf8')
        response = self.client.patch('/users/birthday/', HTTP_AUTHORIZATION=f"Basic {base64_credentials}")
        self.assertEqual(response.status_code, 405)

    def test_get_birthday_correct_auth_noid(self):
        credentials = f"{self.username}:{self.password}"
        base64_credentials = base64.b64encode(credentials.encode('utf8')).decode('utf8')
        response = self.client.get('/users/birthday/', HTTP_AUTHORIZATION=f"Basic {base64_credentials}")
        self.assertEqual(response.status_code, 400)

    # def test_get_birthday_correct_auth_badid(self):
    #     credentials = f"{self.username}:{self.password}"
    #     base64_credentials = base64.b64encode(credentials.encode('utf8')).decode('utf8')
    #     response = self.client.generic('GET', '/users/birthday/', json.dumps({"id": 123456789012345679}), HTTP_AUTHORIZATION=f"Basic {base64_credentials}")
